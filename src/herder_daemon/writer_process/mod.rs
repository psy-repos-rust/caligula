...
fn run(
    ...
    if cfg!(target_os = "macos") && args.target_type == device::Type::Disk {
        ...
        if !exit.success() {
            return Err(WriteVerifyWorkerError::FailedToUnmount {
...
impl<S: Read, D: Write> WriteOp<S, D> {
    ...
    #[inline(always)]
    fn execute(&mut self, mut tx: impl FnMut(WriteVerifyEvent)) -> Result<u64, WriteVerifyWorkerError> {
        ...
        loop {
            for _ in 0..self.checkpoint_period {
                ...
                if written_bytes == 0 {
                    checkpoint!();
                    return Err(WriteVerifyWorkerError::EndOfOutput);
                }
            }
            checkpoint!();
        }
...
impl<S: Read, D: Read> VerifyOp<S, D> {
    #[inline(always)]
    fn execute(&mut self, mut tx: impl FnMut(WriteVerifyEvent)) -> Result<(), WriteVerifyWorkerError> {
        let mut file = FileSourceReader::new(self.cf, self.file_read_buf_size, &mut self.file);
        ...
        loop {
            for _ in 0..self.checkpoint_period {
                ...
                if file_buf[..file_read_bytes] != disk_buf[..file_read_bytes] {
                    ...
                    return Err(WriteVerifyWorkerError::VerificationFailed);
                }
            }
            checkpoint!();
        }
